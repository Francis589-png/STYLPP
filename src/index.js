...
function valueOf(value,env){let v=String(value);const variables=[];v=v.replace(/var\(\s*([A-Za-z_][\w-]*)\s*\)/g,(_,name)=>{const key=`__STYL_VAR_${variables.length}__`;variables.push(`var(--${name})`);return key;});v=v.replace(/\b([A-Za-z_][\w-]*)\b/g,n=>Object.prototype.hasOwnProperty.call(env,n)?env[n]:n);variables.forEach((replacement,index)=>{v=v.replace(`__STYL_VAR_${index}__`,replacement);});if(/^var\(--[A-Za-z_][\w-]*\)$/.test(v.trim()))return v.trim();const parts=v.match(/^(.+?)\s*([+*/-])\s*(.+)$/);if(!parts)return v;const[,left,op,right]=parts,a=parseNumber(left),b=parseNumber(right);if(!a||!b)return`calc(${left.trim()} ${op} ${right.trim()})`;if(op==='+')return a.unit===b.unit?`${a.n+b.n}${a.unit}`:`calc(${left.trim()} + ${right.trim()})`;if(op==='-')return a.unit===b.unit?`${a.n-b.n}${a.unit}`:`calc(${left.trim()} - ${right.trim()})`;if(op==='*'){if(a.unit&&b.unit)return`calc(${left.trim()} * ${right.trim()})`;return`${a.n*b.n}${a.unit||b.unit}`;}if(b.n===0)throw new StylppError('Division by zero');return b.unit?`calc(${left.trim()} / ${right.trim()})`:`${a.n/b.n}${a.unit}`;}
