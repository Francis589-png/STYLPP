...
class Signal {
  ...
  map(fn) { const derived = new Signal(fn(this._value)); this.subscribe(v => { derived.value = fn(v); }); return derived; }
}

class UINode {
  constructor(type = 'view', props = {}) {
    this.type = type; this.props = { ...props }; this.children = []; this.parent = null; this.listeners = new Map();
  ...
  emit(event, detail = {}) { const evt = { type: event, target: this, detail, stopped: false, stopPropagation() { this.stopped = true; } }; let node = this; while (node) { for (const handler of node.listeners.get(event) || []) handler(evt); if (evt.stopped) break; node = node.parent; } return !evt.stopped; }
...
function layout(node, width = px(node.props.width, 0), height = px(node.props.height, 0)) {
  ...
  const direction = node.props.layout || (node.props.direction === 'row' ? 'row' : 'column');
  if (!node.children.length) return node;
  const gap = px(node.props.gap, 0), padding = px(node.props.padding, 0);
  const innerW = Math.max(0, width - padding * 2), innerH = Math.max(0, height - padding * 2);
  if (direction === 'row') {
    const each = Math.max(0, (innerW - gap * (node.children.length - 1)) / node.children.length);
    node.children.forEach((child, i) => { child.layout.x = padding + i * (each + gap); child.layout.y = padding; layout(child, px(child.props.width, each), px(child.props.height, innerH)); });
  } else {
    const each = Math.max(0, (innerH - gap * (node.children.length - 1)) / node.children.length);
    node.children.forEach((child, i) => { child.layout.x = padding; child.layout.y = padding + i * (each + gap); layout(child, px(child.props.width, innerW), px(child.props.height, each)); });
  }
  return node;
}
