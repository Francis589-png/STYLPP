...
class STYLRuntimeError extends Error {
  constructor(message) { super(message); this.name = 'STYLRuntimeError'; }
}

class STYLRuntime {
  constructor({ root, gravity = { x: 0, y: 980 }, width = 0, height = 0 } = {}) {
    this.ui = new UIRuntime(root || new UINode('root'));
    this.physics = new PhysicsWorld({ gravity });
    this.timelines = new Set();
    this.springs = new Set();
    this.signals = new Set();
    this.width = width; this.height = height; this.time = 0;
  }
  ...
  step(dt) {
    if (!Number.isFinite(dt) || dt < 0 || dt > 0.1) throw new STYLRuntimeError('dt must be between 0 and 0.1 seconds');
    this.time += dt;
    for (const spring of this.springs) if (!spring.settle()) spring.step(dt);
    this.physics.step(dt);
    for (const timeline of this.timelines) timeline.step(dt);
    this.ui.tick(this.width, this.height);
