...
class Spring {
  ...
  settle(epsilon = 0.001) {
    epsilon = finite(epsilon, 'epsilon');
    if (epsilon < 0) throw new PhysicsError('epsilon must be >= 0');
...
class Body {
  constructor({ x = 0, y = 0, vx = 0, vy = 0, mass = 1, gravity = { x: 0, y: 980 }, damping = 0 } = {}) {
    ...
    if (this.mass <= 0 || this.damping < 0) throw new PhysicsError('mass must be > 0 and damping must be >= 0');
  }
...
class PhysicsWorld {
  constructor({ gravity = { x: 0, y: 980 }, fixedDt = 1 / 60, maxSubSteps = 8 } = {}) {
    this.gravity = vec(gravity.x, gravity.y);
    this.fixedDt = finite(fixedDt, 'fixedDt');
    this.maxSubSteps = finite(maxSubSteps, 'maxSubSteps');
    if (this.fixedDt <= 0 || this.fixedDt > 0.1) throw new PhysicsError('fixedDt must be > 0 and <= 0.1 seconds');
    if (!Number.isInteger(this.maxSubSteps) || this.maxSubSteps < 1) throw new PhysicsError('maxSubSteps must be a positive integer');
    this.accumulator = 0;
    this.bodies = [];
  }
  add(body) {
    if (!(body instanceof Body)) throw new PhysicsError('world can only add Body instances');
    this.bodies.push(body); body.gravity = vec(this.gravity.x, this.gravity.y); return body;
  }
  remove(body) {
