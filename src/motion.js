...
function clamp(v, min = 0, max = 1) { return Math.min(max, Math.max(min, finite(v, 'value'))); }
...
class Tween {
  ...
  step(dt) {
    ...
    this.elapsed = Math.min(this.duration, this.elapsed + finite(dt, 'dt'));
...
class Timeline {
  ...
  seek(seconds) { this.time = clamp(seconds, 0, this.duration); return this; }
  step(dt) {
    if (!this.playing) return this.time;
    this.time = Math.min(this.duration, this.time + finite(dt, 'dt'));
    if (this.time >= this.duration) this.playing = false;
    return this.time;
  }
  sample() { return this.tracks.map(({ tween, start }) => ({ value: tween.from + (tween.to - tween.from) * easing(tween.curve, clamp((this.time - start) / tween.duration)), active: this.time >= start && this.time <= start + tween.duration })); }
}
